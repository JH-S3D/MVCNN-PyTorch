...
class MVCNN(nn.Module):
    ...
    def __init__(self, embedding_size=4096):
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Conv2d(256, 256, kernel_size=3, padding=1, output_padding=0),
            nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2),  # Increase size

            nn.Conv2d(256, 192, kernel_size=3, padding=1, output_padding=0),
            nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2),  # Further increase size

            nn.Conv2d(192, 64, kernel_size=5, padding=2, output_padding=0),
            nn.ReLU(inplace=True),
            nn.Upsample(scale_factor=2),  # Further increase size
            
            nn.Conv2d(64, 3, kernel_size=11, stride=4, padding=2, output_padding=1),
            nn.ReLU(inplace=True)
        )
